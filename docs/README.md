...
All the openstack services require passwords for authenticating to each service endpoint.  A /etc/kolla/passwords.yml file containing all the service passwords is needed.  Luckily Kolla-ansible includes a tool for managing and generating all these indivual service passwords and for creating the /etc/kolla/passwords.yml file.
